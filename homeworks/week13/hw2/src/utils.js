...
const userAddComment = (siteKey) => {
  
  $(`.btn-submit-${siteKey}`).click(async (e) => {
    ...
    const site = formNode.find('input[name=site]').val();
    const parentNode = $(`.comments__container-${site}`);
    const newCommentData = `nickname=${nickname}&comment=${comment}&site=${site}`
    e.preventDefault();
    ...
    $(parentNode).text('');
    const { siteKey, offset} = commentsPluginData[site]
...
const clickLoveHandler = (siteKey) => {
  $(`.comments__container-${siteKey}`).on('click', '.love', (e) => {
    const allowClickLoadMore = commentsPluginData[siteKey].allowClickLoadMore;
    e.preventDefault();
...
const loadMoreBtnHandler = (siteKey) => {
  $(`.btn-loadmore-${siteKey}`).click(async() => {
    ...
    const offset = commentsPluginData[siteKey].offset
...
const getCommentsRequest = (siteKey, offset) => fetch(`api/comments.php?offset=${offset}&site=${siteKey}`)
  .then(data => data.json())
  .then(data => commentsPluginData[siteKey].commentsData = data)

// append 拿到的留言
const appendComments = (siteKey) => {
  const parentNode = $(`.comments__container-${siteKey}`);
  const { commentsData } = commentsPluginData[siteKey];
  commentsData.forEach(element => $(parentNode).append(template.commentTemplate(element,escapeHtml)));
};

// 新增留言 request
const addCommentRequest = data => fetch('api/comments.php', {
  method: 'POST',
  headers: {
    'content-type': 'application/x-www-form-urlencoded'
  },
...
const loadComments = async (siteKey, offset) => {
  await getCommentsRequest(siteKey, offset);
  console.log(commentsPluginData[siteKey].commentsData)
  appendComments(siteKey);
}
